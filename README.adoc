...
=== WiFi пульт управления

Беспроводное управление роботом через WiFi с веб-интерфейсом и виртуальным джойстиком.
...
[source,cpp]
----
// Инициализация WiFi точки доступа (с параметрами по умолчанию)
void begin(
...
// Получить значения скоростей моторов от пульта (-200...200)
void getMotorValues(int& left, int& right);
// Получить IP адрес точки доступа
String getIPAddress();
// Получить имя WiFi сети (SSID)
String getSSID();
// Получить пароль WiFi сети
String getPassword();
// Получить IP адрес шлюза
String getGateway();
// Получить маску подсети
String getSubnet();
----

Пример использования: link:examples/wifiRemote/wifiRemote.ino[wifiRemote.ino]
...
| WiFi пульт управления | Беспроводной (WiFi)
|===
